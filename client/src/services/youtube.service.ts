...
class YoutubeService {
	...
	async parseFirstVideo(name: string): Promise<IYoutubeResponse> {
		...
		const title = this.getString(result, '"title":{"runs":[{"text":"', '"}],"accessibility":')
		const videoId = this.getString(result, '"videoRenderer":{"videoId":"', '","thumbnail":')
		const thumbnail = this.getString(result, '{"thumbnails":[{"url":"', '","width"')
